...
import { generateVolunteerMessage } from '../services/geminiService';
...
const VolunteerDetailPage: React.FC<{ overrideUser?: UserProfile, setCurrentUser?: (user: UserProfile | null) => void }> = ({ overrideUser, setCurrentUser }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const aiMessage = await generateVolunteerMessage(
        user.name,
        messageText,
        user.skills || [],
        user.bio || ''
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 animate-in slide-in-from-bottom duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <section className="bg-slate-900 text-white p-12 rounded-[3rem] space-y-8 relative">
          {messageSent && (
            <div className="absolute inset-0 bg-green-600 rounded-[3rem] p-12 z-10 flex flex-col items-center justify-center text-center space-y-6 animate-in fade-in duration-300">
              <svg className="w-20 h-20 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <h3 className="text-3xl font-black">Message Sent! 🐙</h3>
              <p className="text-white/90 max-w-md">Your AI-crafted message has been delivered to {user.name}. They'll get back to you soon!</p>
              <div className="bg-white/10 p-6 rounded-2xl max-w-lg backdrop-blur-sm">
                <p className="text-sm text-white/80 italic">"{generatedMessage.substring(0, 200)}..."</p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-4">
            <textarea 
              ...
              className="w-full bg-slate-800 border-none rounded-2xl p-6 text-white focus:ring-2 focus:ring-brand h-40 disabled:opacity-50"
              placeholder="Write your message here..."
            ></textarea>
            <button 
              onClick={handleSendMessage}
              disabled={isSendingMessage || !messageText.trim()}
              className="w-full py-5 bg-brand text-white font-black rounded-3xl shadow-xl shadow-brand/20 hover:bg-brand-dark transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
            >
              {isSendingMessage ? (
                <>
