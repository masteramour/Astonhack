...
export async function fetchVolunteerById(id: number) {
  try {
    ...
    throw new Error(`Failed to fetch volunteer with ID ${id}`);
  }
}
...
export async function modifyEvent(id: number, data: Partial<{
  ...
  try {
    const dbData: Partial<Omit<DBEvent, 'Event_ID'>> = {
      Title: data.name, // Corrected property name
      Manager_ID: data.managerId,
      Date: data.date,
      VolunteersNeeded: data.volunteersNeeded, // Corrected property name
