...
const DonatePage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12">
      ...
      {/* Donor List Section */}
      <div className="bg-white dark:bg-slate-800 rounded-[3rem] p-10 shadow-2xl">
        <h2 className="text-2xl font-bold mb-6">Recent Donors</h2>
        <ul className="space-y-4">
          {donations.map((donor, index) => (
            <li key={index} className="flex justify-between items-center p-4 bg-slate-100 dark:bg-slate-700 rounded-2xl">
              <span className="font-medium text-slate-700 dark:text-slate-300">{donor.fullName}</span>
              <span className="text-slate-500 dark:text-slate-400">{donor.category}: {donor.description}</span>
            </li>
          ))}
        </ul>
      </div>
